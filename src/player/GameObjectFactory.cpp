...
GameObject* GameObjectFactory::create(const std::string &in)
{
    ...
    case SHIP3:
        ...
        gameObject = new GOM_Ship(ObjectType(type), objID, degree, posX,
                                  posY, playerID, speed, maxHealth, attackPower,maxSpeed);
    ...
    case POWERUP:
        ...
        gameObject = new GOS_PowerUp(ObjectType(type), objID, degree,
                                     posX, posY, playerID, speed, ttl,
                                     damage, GOS_PowerUp::BONUS(bonus));
        gameObject->setHitBox(tl, tr, bl, br);
...
int GameObjectFactory::getObjectID(const std::string &str)
{
    ...
    case SHIP1:
    case SHIP2:
        istr >> objID >> degree >> posX >> posY >> playerID >> speed >> maxspeed >> health
             >> attackPower
             >> tl_x >> tl_y
             >> tr_x >> tr_y
             >> bl_x >> bl_y
